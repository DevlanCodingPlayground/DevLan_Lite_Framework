...
if (isset($_GET['status'])) {
    ...
    if ($_GET['status'] == 'cancelled') {
        header('Location: staff_dashboard_sales?action=empty');
        ...
        if ($res->status) {
            ...
            if ($amountPaid >= $amountToPay) {
                ...
                $payment_receipt_number = $_GET['receipt_number'];
                /* Get Cart Items Via Session */
                ...
                $sql = "INSERT INTO payments (payment_receipt_number, payment_amount, payment_txn_code, payment_date_posted)
                VALUES(?,?,?,?)";
                $sale = "UPDATE sales SET sale_payment_status = ? WHERE sale_receipt_no =?";

                $prepare = $mysqli->prepare($sql);
                $sale_prepare = $mysqli->prepare($sale);
                ...
                if ($prepare && $sale_prepare) {
                    /* Load Cart Session Via Details */
                    foreach ($cart_products as $cart_products) {
                        ...
                        if (mysqli_num_rows($res) > 0) {
                            ...
                            if ($products['product_quantity'] > $sale_quantity) {
                                ...
                                $sale_sql =  "INSERT INTO sales (sale_product_id, sale_user_id, sale_quantity, sale_customer_name, sale_customer_phoneno, sale_receipt_no, sale_payment_method, sale_payment_status)
                                VALUES(?,?,?,?,?,?,?,?)";
                                ...
                                $success = "Sale Posted" &&
                                    header('Location: main_dashboard_sales_receipt?print_receipt=' . $payment_receipt_number . '');
                    ...
                    $success = "Sale Posted" &&
                        header('Location: staff_dashboard_receipt?print_receipt=' . $payment_receipt_number . '');
                }
            } else {
