...
/* Excel File Name */
$fileName = "Stock Additions From $start To $end .xls";

/* Excel Column Name */
$fields = array('Item Details', 'Quantity Added', 'Date Added');
...
if ($query->num_rows > 0) {
    ...
    while ($row = $query->fetch_assoc()) {
        /* Sanitize Log Date */
        $date_added = date('d M Y g:ia', strtotime($row['inventory_date_added']));
        /* Hardwire This Data Into .xls File */
        $lineData = array($row['product_name'] . ' ' . $row['product_name'], $row['inventory_qty_added'],  $date_added);
        array_walk($lineData, 'filterData');
        $excelData .= implode("\t", array_values($lineData)) . "\n";
    }
} else {
    $excelData .= 'No Stock Additions Available...' . "\n";
}
