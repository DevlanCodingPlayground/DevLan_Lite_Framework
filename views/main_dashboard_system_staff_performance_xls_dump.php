...
/* Excel File Name */
$fileName = "Staff perfomance Report.xls";

/* Excel Column Name */
$fields = array('Staff Name', 'Store Name', 'No of Sale','Total Revenue');
...
$query = $mysqli->query("SELECT us.user_id, us.user_name,ss.store_name, COUNT(*)AS count,SUM(p.product_sale_price-s.sale_discount*s.sale_quantity) AS total_cost  FROM sales s 
...
if ($query->num_rows > 0) {
    ...
    while ($row = $query->fetch_assoc()) {
        ...
        $lineData = array($row['user_name'], $row['store_name'], $row['count'],$row['total_cost']);
        array_walk($lineData, 'filterData');
        $excelData .= implode("\t", array_values($lineData)) . "\n";
    }
} else {
    $excelData .= 'Staff Perfomance Records Available...' . "\n";
}
