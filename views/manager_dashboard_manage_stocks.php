...
if (isset($_POST['update_product_stock'])) {
    ...
    /* Activity Logged */
    $log_type = "Added new stock of $product_quantity new items to $product_details";
    ...
    while ($user = $res->fetch_object()) {
        ...
        if ($user_access_level != 'Staff') {
            ...
            while ($product = $res->fetch_object()) {
                ...
                $inventory_sql  = "INSERT INTO inventory(inventory_product_id, inventory_qty_added)
                VALUES('{$product_id}', '{$product_quantity}')";
                $stmt2 = $mysqli->prepare($query2);
                $inventory_smt = $mysqli->prepare($inventory_sql);
                $rc2 = $stmt2->bind_param('ss',  $new_stock, $product_id);
                $stmt2->execute();
                $inventory_smt->execute();
                /* Load Log Helper */
