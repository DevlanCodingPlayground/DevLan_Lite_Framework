...
/* Excel Column Name */
$fields = array('Item Code', 'Item Name', 'Current Quantity', 'Purchase Price (Ksh)', 'Retail Price(Ksh)', 'Margin (Ksh)');
...
$query = $mysqli->query("SELECT * FROM products  WHERE product_status = 'active'
ORDER BY product_name ASC");
if ($query->num_rows > 0) {
    /* Load All Fetched Rows */
    while ($row = $query->fetch_assoc()) {
        /* Sanitize Log Date */
        $margin = ($row['product_sale_price'] - $row['product_purchase_price']);
        /* Hardwire This Data Into .xls File */
        $lineData = array($row['product_code'], $row['product_name'], $row['product_quantity'], $row['product_purchase_price'], $row['product_sale_price'], $margin);
        array_walk($lineData, 'filterData');
        $excelData .= implode("\t", array_values($lineData)) . "\n";
    }
} else {
    $excelData .= 'No Products Available...' . "\n";
}
