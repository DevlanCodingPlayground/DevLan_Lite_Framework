...
while ($stores = $res->fetch_object()) {
    ...
    <html>
        ...
        <body style="margin:1px;">
            ...
            <div class="list_header" align="center">
                ...
                <h4> Customer Purchases Report From ' . $start . ' To ' . $end . ' </h4>
            </div>
            <table border="1" cellspacing="0" width="98%" style="font-size:9pt">
            <thead>
                <tr>
                    <th style="width:100%">Customer Name</th>
                    <th style="width:50%">Customer Phone</th>
                    <th style="width:50%">No of Items</th>
                    <th style="width:70%">Total Expenditure</th>
                   
                </tr>
            </thead>
            ';
    $ret = "SELECT sale_customer_name,sale_customer_phoneno,COUNT(*)AS count,SUM(p.product_sale_price*s.sale_quantity) AS total_cost ,sale_datetime  
            FROM sales s
            LEFT JOIN products p ON  p.product_id =s.sale_product_id
            INNER JOIN users us ON us.user_id = s.sale_user_id
            WHERE us.user_store_id ='$user_store_id' AND s.sale_datetime BETWEEN '$start' AND '$end' 
    ...
    while ($customers = $res->fetch_object()) {
        ...
        $html .=
            '
                    <tr>
                        <td>' . $customers->sale_customer_name . '</td>
                        <td>' . $customers->sale_customer_phoneno . '</td>
                        <td>' . $customers->count . '</td>
                        <td> Kshs' . $customers->total_cost  . '</td>
    ...
    $dompdf->render();
    $dompdf->stream('Customer Perfomance From ' . $start . ' To ' . $end, array("Attachment" => 1));
