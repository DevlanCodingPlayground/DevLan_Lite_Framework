...
/* Excel File Name */
$fileName = "Customer perfomance Report.xls";

/* Excel Column Name */
$fields = array('Customer Name', 'Customer Phone', 'No of Items','Total Expenditure');
...
$query = $mysqli->query("SELECT sale_customer_name,sale_customer_phoneno,COUNT(*)AS count,SUM(p.product_sale_price*s.sale_quantity) AS total_cost ,sale_datetime  
FROM sales s
...
if ($query->num_rows > 0) {
    ...
    while ($row = $query->fetch_assoc()) {
        ...
        $lineData = array($row['sale_customer_name'], $row['sale_customer_phoneno'], $row['count'],$row['total_cost']);
        array_walk($lineData, 'filterData');
        $excelData .= implode("\t", array_values($lineData)) . "\n";
    }
    
} else {
    $excelData .= 'Customer Records Available...' . "\n";
}
